...
def generate_tvm_libs(dataset):
    cmd = [TVM_LIB_RUNNER, dataset, '1' if args.bin_packed else '0', '0']
    print(' '.join(cmd))
    out, err = run_cmd(cmd)
    print(err)
...
results_out, results_err = get_out_files(args, 'bert_layer', 'a' if args.append else 'w')
...
for target in targets:
    for _, dataset_list in datasets.items():
        if args.gen_libs: generate_tvm_libs(dataset_list[0]);
