...
QKT_BP2_TVM_RUNNER = SCRIPT_DIR + '/../bert_layer/tvm/qkt_bin_packed.py'
QKT_BP_TVM_RUNNER = SCRIPT_DIR + '/../bert_layer/tvm/qkt_bin_packed2.py'
...
def get_runner_and_args(op, split_op, hfuse, split2):
    if op == 'qkt':
        if split2:
            return QKT_BP2_TVM_RUNNER, ['--hfuse'] if hfuse else []
        else:
            if split_op:
                return QKT_BP_TVM_RUNNER, ['--hfuse'] if hfuse else []
            else:
                return QKT_BP_TVM_RUNNER, ['--no-split']
    elif op == 'attn_v':
        if split_op:
...
header = 'Op,Target,Dataset,Batch Size,Vanilla Time,+OpSplit,+HFuse,CBT,SplitHFuse2'
...
for op in ops:
    for dataset in datasets:
        ...
        hfuse2_times = {}
        if op == 'qkt':
            hfuse2_times = prepare_and_execute(op, args.target, dataset, b_sizes, args.max_batches, True, True, True)
        print(hfuse2_times)
