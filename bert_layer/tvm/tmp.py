...
loop_ufs=[ls[5], ls[0], ls[1], ls[4], ls[2]]
width_ufs = None if args.dense_storage else [ls[5], ls[0], ls[1], ls[4], sufw.get_uf()]
Q = te.ragged_placeholder((3, BATCH_SIZE, NUM_HEADS, HEAD_SIZE, MAX_LEN), [qk, bd, md, hd, s1], loop_ufs,
                          name='Q', width_ufs=width_ufs)

loop_ufs=[ls[5], ls[0], ls[1], ls[4], ls[3]]
width_ufs = None if args.dense_storage else [ls[5], ls[0], ls[1], ls[4], sufw.get_uf()]
K = te.ragged_placeholder((3, BATCH_SIZE, NUM_HEADS, HEAD_SIZE, MAX_LEN), [qk, bd, md, hd, s2], loop_ufs,
...
S = te.ragged_compute((BATCH_SIZE, MAX_LEN, NUM_HEADS, MAX_LEN), [bd, s1, md, s2], loop_ufs,
                      lambda ds: tvm.sum(Q[0, ds[bd], ds[md], k, ds[s1]] * K[1, ds[bd], ds[md], k, ds[s2]],
...
if args.target == "cuda":
    ...
    h, x, y = s[Ks].leaf_iter_vars[2], s[Ks].leaf_iter_vars[3], s[Ks].leaf_iter_vars[4]
    s[Ks].bind(x, thread_y())
    fio, fii = s[Ks].split(y, factor = nt * 4)
    fiio, fiii = s[Ks].split(fii, factor = 4)
    s[Ks].bind(fiio, thread_x())
    s[Ks].vectorize(fiii)

    h, x, y = s[Qs].leaf_iter_vars[2], s[Qs].leaf_iter_vars[3], s[Qs].leaf_iter_vars[4]
    s[Qs].bind(x, thread_y())
    fio, fii = s[Qs].split(y, factor = nt * 4)
