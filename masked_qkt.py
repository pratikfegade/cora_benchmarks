...
def len1_uf(name): return Uf(name, (64, MAX_LEN), [bd], lambda b: utils.ceilmult(lens[b], TILE1))
def len2_uf(name): return Uf(name, (64, MAX_LEN), [s1], lambda s: utils.ceilmult(s + 1, TILE2))
...
O = te.ragged_compute((BATCH_SIZE, NUM_HEADS, MAX_LEN, MAX_LEN), [bd, md, s1, s2], loop_ufs,
                        lambda ds: tvm.sum(Q[ds[bd], ds[md], k, ds[s1]] * K[ds[bd], ds[md], k, ds[s2]], axis = k),
