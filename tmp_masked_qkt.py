...
def len1_uf(name): return Uf(name, 'l', (64, MAX_LEN), [bd], lambda b: utils.ceilmult(lens[b], TILE1))
def len2_uf(name): return Uf(name, 'l', (64, MAX_LEN), [s1], lambda s: utils.ceilmult(s + 1, TILE2))
...
# elif args.target == 'c' or args.target == 'llvm':
#     b, h, x, y = s[O].leaf_iter_vars[0:4]
#     # yo, yi = s[O].split(y, factor = 64)
#     # xo, xi = s[O].split(x, factor = 64)
...
if args.debug_code:
    ...
    fadd = tvm.runtime.module.load_module('/home/ppf/data/ppf/projects/projects/ragged_tensors/incubator-tvm/build/masked_qkt.so')
