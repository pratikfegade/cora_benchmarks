...
def len_uf(name, pad): return Uf(name, (64, 128), [bd], lambda b: utils.ceilmult(lens[b], pad))
...
O = te.ragged_compute((BATCH_SIZE, MAX_LEN, HEAD_SIZE), [bd, s1, hd], loop_ufs,
                      lambda ds: A[ds[bd], ds[s1], ds[hd]] * 2, name = 'O', width_uf_lists=[width_ufs])
...
lo, li = s[O].split(l, factor = 64)
f = s[O].fuse(b, lo)
s[O].bind(f, block_x)
s[O].bind(li, thread_x)
